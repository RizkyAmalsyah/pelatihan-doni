            ...
            <div class="col-md-4 col-lg-3">
                <div class="widget">
                    @if($setting->logo_white  && file_exists(public_path('data/setting/' . $setting->logo_white )))
                         <div class="background-partisi-contain" style="width : 200px;height : 80px;background-image : url('{{ image_check($setting->logo_white,'setting') }}');"></div>
                    @endif
                    @php
                        $status = false;
                ...
                <div class="widget">
                    ...
                    <div class="newsletter-wrapper">
                        ...
                        <div id="mc_embed_signup2">
                            <form id="form_subscribe" action="{{ route('subscribe.insert') }}" method="post" class="validate dark-fields" novalidate>
                                <div id="req_subscribe_email">
                                    <div class="mc-field-group input-group form-floating">
                                        <input type="email" name="email"
                                            class="required email form-control" placeholder="Alamat Email"
                                            id="email-subscribe" autocomplete="off">
                                        <label for="email-subscribe">Alamat Email</label>
                                        <button class="btn btn-primary" id="button_subscribe" type="button" onclick="submit_form(this,'#form_subscribe', '', false, false,`<div class='spinner-border text-light' role='status'></div>`)">
            ...
            <div class="modal-body">
                ...
                <form id="formRegister" class="pt-5 d-none" method="POST" action="{{ route('auth.register') }}">
                    <div id="pane_register">
                        ...
                        <button role="button" id="button_regis_1" onclick="submit_form(this,'#formRegister', '', false, false,'',`{{ route('auth.register.first') }}`)" class="btn btn-primary w-100 text-white rounded-3 shadow-sm hover-shadow">Selanjutnya</button>

                    </div>

                    <div id="pane_vector" class="d-none">
                        <div class="mb-3 text-center">
                            <h2 class="form-label">Pilih minat anda</h2>
                        </div>
                        <div class="row w-100">
                            ...
                            <div class="col-md-6 col-sm-12 my-3">
                                <div class="form-check">
                                    <input class="cursor-pointer form-check-input checkbox-setup" name="vector[]" type="checkbox" value="{{ $row->id_vector }}" id="vector-{{ $row->id_vector }}" />
                                    <label class="cursor-pointer form-check-label" for="vector-{{ $row->id_vector }}">
                                        {{ $row->name }}
                                    </label>
                                </div>
                            </div>
                        ...
                        <div class="w-100 d-flex row">
                            <div class="col-md-6 d-flex justify-content-center align-items-center mb-2">
                                 <button role="button" id="cancel_register" onclick="cancel_register()" class="btn btn-secondary w-100 text-white rounded-3 shadow-sm hover-shadow">Kembali</button>
                            </div>
                            <div class="col-md-6 d-flex justify-content-center align-items-center mb-2">
...
<div class="modal fade" id="modalDetailTraining" data-bs-backdrop="static" data-bs-keyboard="false" aria-hidden="true" aria-labelledby="modalDetailTrainingLabel" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered {{ (session(config('session.prefix') . '_id_user')) ? 'modal-xl' : 'modal-lg' }}">
        <div class="modal-content rounded">
            <div class="modal-body">
                <div class="w-100 d-flex justify-content-end align-items-center px-4">
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"><i class="fa-solid fa-xmark"></i></button>
                ...
                <div class="row">
                    <div class="{{ (session(config('session.prefix') . '_id_user')) ? 'col-lg-7 col-md-12' : 'col-12' }}">
                        ...
                        <div class="d-flex justify-content-start align-items-start flex-column">
                            <div class="rounded background-partisi w-100 mb-4 d-flex justify-content-start align-items-start" id="display_image_training" style="height : 300px;background-image : url({{ image_check('default','training') }})">
                                <span class="bg-primary px-3 text-white fs-15 rounded mx-3 my-4" id="display_category_training"></span>
                            </div>
                    ...
                    <div class="col-lg-5 col-md-12">
                        <form class="contact-form needs-validation" id="form_pendaftaran_pelatihan" method="post" action="{{ route('register.training') }}" novalidate >
                            ...
                            <div class="row gx-4">
                                ...
                                <div class="col-md-12">
                                    <div class="form-floating mb-4" id="req_regis_training_field_{{ $row->id_form }}">
                                        <input id="form_pendaftaran_{{ $row->id_form }}" type="{{ set_form_type($row->type) }}" name="field_{{ $row->id_form }}" class="form-control" placeholder="Enter {{ $row->field }}" required autocomplete="off">
                                        <label for="form_pendaftaran_{{ $row->id_form }}">{{ $row->field }} *</label>
                                    </div>
                                </div>
                                @elseif($row->type == 4)
                                <div class="col-md-12">
                                    <label style="color : #969eab;" for="form_pendaftaran_{{ $row->id_form }}">{{ $row->field }} *</label>
                                    <div class="form-floating mb-4" id="req_regis_training_field_{{ $row->id_form }}">
                                        <input id="form_pendaftaran_{{ $row->id_form }}" type="{{ set_form_type($row->type) }}" name="field_{{ $row->id_form }}" class="form-control" placeholder="Enter {{ $row->field }}" required autocomplete="off">
                                    </div>
                                </div>
                                ...
                                <div class="col-md-12">
                                    <div class="form-floating mb-4" id="req_regis_training_field_{{ $row->id_form }}">
                                        <textarea name="field_{{ $row->id_form }}" id="form_pendaftaran_{{ $row->id_form }}" cols="30" rows="10" class="form-control" placeholder="Enter {{ $row->field }}"></textarea>
                                        <label for="form_pendaftaran_{{ $row->id_form }}">{{ $row->field }} *</label>
                                    </div>
                                </div>
                                @endif
                                ...
                                <div class="col-12 d-flex justify-content-center align-items-center">
                                    <button type="button" id="submit_form_pendaftaran_pelatihan" onclick="submit_form(this,'#form_pendaftaran_pelatihan')" class="btn btn-primary rounded-pill btn-send mb-3">Kirim</button>
                                </div>
                                <!-- /column -->
                            </div>
                            <!-- /.row -->
