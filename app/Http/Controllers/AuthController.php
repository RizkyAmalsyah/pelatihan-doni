...
class AuthController extends Controller
{
    ...
    public function register(Request $request)
    {
        ...
        if (!$vector) {
            return response()->json([
                'status' => 500,
                'alert' => ['message' => 'Minat tidak boleh kosong! Minimal isi salah satu checklist untuk melanjutkan'],
            ]);
        }else{
            $cek = Vector::whereIn('id_vector',$vector)->get();
            if (!$cek || $cek->isEmpty()) {
                return response()->json([
                    'status' => 500,
                    'alert' => ['message' => 'Minat tidak boleh kosong! Minimal isi salah satu checklist untuk melanjutkan'],
        ...
        if ($user) {
            ...
            if ($cek && $cek->isNotEmpty()) {
                $push = [];

                foreach ($cek as $key) {
                    $push[] = [
                        'id_user' => $user->id_user,
                        'id_vector' => $key->id_vector,
                    ];
                }

                UserVector::insert($push);
            }
